...
const TRANSLATIONS = {
  uz: {
    ...
    viewOrders: "Buyurtmalarni ko'rish", exportBtn: "Eksport",
    // Clients
  ...
  ru: {
    ...
    viewOrders: "Смотреть заказы", exportBtn: "Экспорт",
...
const mk = (C, L) => ({
  ...
  topRight: { display: "flex", alignItems: "center", gap: 8, flexWrap: "nowrap" },
  themeBtn: { background: L ? "#e8edf8" : "#ffffff15", border: `1px solid ${C.border}`, borderRadius: 20, padding: "5px 11px", fontSize: 12, cursor: "pointer", color: C.text },
  langBtn: (active) => ({ background: active ? `${C.accent}20` : "transparent", border: `1px solid ${active ? C.accent : C.border}`, borderRadius: 16, padding: "4px 10px", fontSize: 12, cursor: "pointer", color: active ? C.accent : C.muted, fontWeight: active ? 700 : 400 }),
...
const INIT_CLIENTS = [
  { id: 1, name: "Ozodbek", country: "🇺🇿 Uzbekistan", cashDebt: 12000, wireDebt: 8500,
    prices: Object.fromEntries(ALL_PRODUCTS.map(p => [p.code, parseFloat((p.single * 3.8).toFixed(2))])) },
  { id: 2, name: "Abu Tashkent", country: "🇺🇿 Uzbekistan", cashDebt: 0, wireDebt: 0,
    prices: Object.fromEntries(ALL_PRODUCTS.map(p => [p.code, parseFloat((p.single * 4.0).toFixed(2))])) },
  { id: 3, name: "Xushnudbek", country: "🇺🇿 Uzbekistan", cashDebt: 30000, wireDebt: 22000,
    prices: Object.fromEntries(ALL_PRODUCTS.map(p => [p.code, parseFloat((p.single * 3.6).toFixed(2))])) },
  { id: 4, name: "Farhodjon", country: "🇨🇳 China", cashDebt: 7200, wireDebt: 5400,
    prices: Object.fromEntries(ALL_PRODUCTS.map(p => [p.code, parseFloat((p.single * 3.5).toFixed(2))])) },
  { id: 5, name: "Azizbek", country: "🇺🇿 Uzbekistan", cashDebt: 10500, wireDebt: 9000,
    prices: Object.fromEntries(ALL_PRODUCTS.map(p => [p.code, parseFloat((p.single * 3.9).toFixed(2))])) },
];
...
function PricelistModal({ clients, onClose }) {
  const C = useC(); const T = useT(); const L = C.bg === LIGHT.bg;
  const S = mk(C, L);
    ...
    <div style={S.overlay} onClick={onClose}>
      <div style={{ ...S.modal, maxWidth: 700 }} onClick={e => e.stopPropagation()}>
        ...
        <div style={S.modalBody}>
          ...
          <div style={{ fontSize: 12, color: C.yellow, background: `${C.yellow}15`, border: `1px solid ${C.yellow}40`, borderRadius: 8, padding: "8px 12px", marginBottom: 14 }}>
            ⚠️ {T.taxNote}
          </div>
          {client && (
            <>
              <div style={{ fontWeight: 600, fontSize: 14, marginBottom: 10 }}>
                {T.previewTitle}: <span style={{ color: C.accent }}>{client.name}</span>
              </div>
              {/* Aluminum section */}
              <div style={{ fontWeight: 700, fontSize: 13, color: C.yellow, marginBottom: 6, marginTop: 4 }}>— {T.aluminum} ({PRODUCTS_ALUM.length} {T.model})</div>
              <div style={{ overflowX: "auto", marginBottom: 12 }}>
                <table style={{ ...S.table, fontSize: 12 }}>
                  <thead>
                    <tr>
                      {["№", T.model, T.rawWeight, T.singleWeight, T.cashNote+" ($)", T.wireNote+" ($)", T.specMm, T.heatKw].map(h =>
                        <th key={h} style={{ ...S.th, padding: "7px 8px", fontSize: 10 }}>{h}</th>
                      )}
                    </tr>
                  </thead>
                  <tbody>
                    {PRODUCTS_ALUM.map((p, i) => {
              ...
              {/* Bimetal section */}
              <div style={{ fontWeight: 700, fontSize: 13, color: C.cyan, marginBottom: 6 }}>— {T.bimetal} ({PRODUCTS_BIMETAL.length} {T.model})</div>
      ...
      <div style={S.modal} onClick={e => e.stopPropagation()}>
        ...
        <div style={S.modalBody}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 14 }}>
            ...
            <div style={{ background: C.inputBg, border: `1px solid ${C.border}`, borderRadius: 10, padding: "10px 12px" }}>
              <div style={{ fontSize: 11, color: C.muted, marginBottom: 3 }}>{T.product.toUpperCase()} · {T.qty.toUpperCase()}</div>
              <div style={{ fontWeight: 700, fontSize: 14 }}>{order.product} <span style={{ color: C.muted, fontWeight: 400 }}>×{order.qty}</span></div>
            </div>
          </div>
          ...
          <div style={{ fontSize: 12, color: C.muted, marginBottom: 5 }}>{T.paymentProgress} · {pct}%</div>
          <div style={{ height: 10, borderRadius: 5, background: C.border, display: "flex", overflow: "hidden", marginBottom: 6 }}>
            <div style={{ width: `${cashPct}%`, background: C.green, transition: "width 0.5s" }} />
            <div style={{ width: `${wirePct}%`, background: C.cyan }} />
          </div>
          <div style={{ display: "flex", gap: 14, fontSize: 12, color: C.muted, marginBottom: 16 }}>
            <span style={{ color: C.green }}>● {T.cashPaid} {cashPct}%</span>
            <span style={{ color: C.cyan }}>● {T.wirePaid} {wirePct}%</span>
            <span>○ {T.remaining} {100-pct}% (${(order.total-order.cashPaid-order.wirePaid).toLocaleString()})</span>
          </div>
          <div style={S.divider} />
          <div style={{ fontSize: 12, color: C.muted, marginBottom: 8, fontWeight: 500 }}>{T.docLang}</div>
          <div style={{ display: "flex", gap: 6, marginBottom: 14 }}>
            {[["uz","🇺🇿 UZ"],["ru","🇷🇺 RU"],["zh","🇨🇳 ZH"]].map(([k,v]) => (
              <button key={k} onClick={() => setDocLang(k)} style={S.langBtn(docLang === k)}>{v}</button>
            ))}
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
            ...
            <button style={S.btn(C.green)}>💳 {TDoc.addPayment}</button>
          </div>
        </div>
      </div>
    </div>
...
function Dashboard() {
  ...
  const totalPaid = INIT_ORDERS.reduce((s, o) => s + o.cashPaid + o.wirePaid, 0);
  const totalDebt = INIT_CLIENTS.reduce((s, c) => s + c.cashDebt + c.wireDebt, 0);
  const active = SHIPMENTS.filter(s => s.status !== "arrived").length;
  return (
    <div>
      <div style={S.pageTitle}>{T.dashboard}</div>
      <div style={S.subtitle}>{T.factoryOverview} — 2026/04</div>
      <div style={S.grid4}>
        {[
          { label: T.totalRevenue, value: `$${(totalRevenue/1000).toFixed(0)}K`, sub: `${INIT_ORDERS.length} ${T.orders_count}`, color: C.accent },
          { label: T.totalCollected, value: `$${(totalPaid/1000).toFixed(0)}K`, sub: `${Math.round(totalPaid/totalRevenue*100)}% ${T.collected}`, color: C.green },
            ...
            <tbody>
              {INIT_ORDERS.slice(0,4).map(o=>{
                ...
                return(<tr key={o.id}>
                  ...
                  <td style={{...S.td,minWidth:80}}><div style={{marginBottom:4,fontWeight:600,fontSize:12}}>{pct}%</div><div style={S.progress()}><div style={S.progressFill(pct,sc)}/></div></td>
      ...
      <div style={S.card}>
        ...
        {SHIPMENTS.map(s=>{
          ...
          const label={ordered:T.ordered,in_transit:T.inTransit,customs:T.atCustoms,arrived:T.arrived}[s.status];
          return(<div key={s.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"12px 0",borderBottom:`1px solid ${C.border}`}}>
            <div><div style={{fontWeight:700}}>{s.id} · <span style={{color:C.muted,fontWeight:400}}>{s.from}</span></div><div style={{fontSize:12,color:C.muted,marginTop:3}}>{s.items}</div></div>
            <div style={{textAlign:"right",flexShrink:0,marginLeft:10}}><span style={S.badge(sc)}>{label}</span><div style={{fontSize:11,color:C.muted,marginTop:5}}>ETA {s.eta}</div></div>
          </div>);
        })}
      </div>
    </div>
...
function Orders() {
  ...
  const filtered = filter === "all" ? INIT_ORDERS : INIT_ORDERS.filter(o => o.status === filter);
  return (
    <div>
      {selected && <OrderModal order={selected} onClose={() => setSelected(null)} />}
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
        <div style={S.pageTitle}>{T.orders}</div>
        <button style={S.btnPrimary}>{T.newOrder}</button>
      </div>
      <div style={S.subtitle}>{T.clickRow}</div>
      <div style={{ display: "flex", gap: 8, marginBottom: 14, flexWrap: "wrap" }}>
        {[["all",C.accent],["paid",C.green],["partial",C.yellow],["unpaid",C.red]].map(([f,col])=>(
          <button key={f} onClick={()=>setFilter(f)} style={{...S.btn(col),fontWeight:filter===f?700:500,opacity:filter===f?1:0.65}}>{T[f]}</button>
        ))}
      </div>
      <div style={{ ...S.card, padding: 0, overflow: "hidden" }}>
        <div style={{ overflowX: "auto" }}>
          <table style={S.table}>
            <thead><tr>
              {[T.orderId,T.client,T.product,T.qty,T.total,T.cashPaid,T.wirePaid,T.remaining,T.progress,T.status].map(h=><th key={h} style={S.th}>{h}</th>)}
            </tr></thead>
            <tbody>
              {filtered.map(o=>{
                ...
                const sc=o.status==="paid"?C.green:o.status==="partial"?C.yellow:C.red;
                return(<tr key={o.id} onClick={()=>setSelected(o)} style={{cursor:"pointer",transition:"background 0.12s"}}
...
function Debts() {
  ...
  const shown = filterClient === "all" ? INIT_CLIENTS : INIT_CLIENTS.filter(c => c.name === filterClient);
    ...
    <div>
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        <select style={S.select} value={filterClient} onChange={e=>setFilterClient(e.target.value)}>
          ...
          {INIT_CLIENTS.map(c=><option key={c.id} value={c.name}>{c.name}</option>)}
        </select>
      </div>
      {shown.map(c=>{
    ...
    <div>
      ...
      {showAdd&&(
        <div style={{...S.card,marginBottom:14,border:`2px solid ${C.accent}60`}}>
          ...
          <div style={S.col}>
            <div><label style={S.label}>{T.originCountry}</label><input style={S.input} placeholder="e.g. China, Turkey..."/></div>
            <div><label style={S.label}>{T.items}</label><input style={S.input} placeholder="Aluminum billets, fittings..."/></div>
            <div style={S.grid2}>
              <div><label style={S.label}>{T.eta}</label><input style={S.input} type="date"/></div>
              <div><label style={S.label}>{T.statusLabel}</label>
                <select style={S.select}><option value="ordered">{T.ordered}</option><option value="in_transit">{T.inTransit}</option><option value="customs">{T.atCustoms}</option><option value="arrived">{T.arrived}</option></select>
              </div>
            </div>
            <div style={{display:"flex",gap:8}}><button style={S.btnPrimary}>{T.save}</button><button style={S.btn(C.muted)} onClick={()=>setShowAdd(false)}>{T.cancel}</button></div>
          </div>
        </div>
      )}
      {SHIPMENTS.map(s=>{
        const meta=statusMeta[s.status];
        return(<div key={s.id} style={{...S.card,marginBottom:12}}>
          ...
          <div style={{display:"flex",gap:8,marginTop:12}}>
            <button style={S.btn(C.accent)}>{T.updateStatus}</button>
...
function Clients() {
  ...
  const saveEdit = (id) => { setClients(p=>p.map(c=>c.id===id?{...c,prices:{...editPrices}}:c)); setEditingId(null); };
            ...
            {isExpanded&&(
              <div style={{marginTop:14}}>
                ...
                <div style={{fontSize:13,color:C.muted,marginBottom:10,fontWeight:500}}>
                  {isEditing ? T.editingPrices : T.currentPrices}
                </div>
                <div style={{overflowX:"auto"}}>
                      ...
                      {prods.map(p=>{
                        ...
                        return(<tr key={p.code}>
                          ...
                          <td style={{...S.td,textAlign:"right"}}>
                            {isEditing
                              ? <span style={{color:C.muted,textDecoration:"line-through",fontSize:12}}>${c.prices[p.code]}</span>
                              : <span style={{fontWeight:700,color:C.green}}>${cash}</span>}
                          </td>
    ...
    <div>
      ...
      <div style={{...S.card,padding:0,overflow:"hidden"}}>
        <div style={{overflowX:"auto"}}>
          <table style={S.table}>
            <thead><tr>
              {["№",T.model,T.rawWeight+"/KG",T.singleWeight+"/KG",T.specMm,T.actualMm,T.heatKw].map(h=><th key={h} style={S.th}>{h}</th>)}
            </tr></thead>
            <tbody>
              {prods.map((p,i)=>(
                <tr key={p.code}
...
export default function App() {
  ...
  const pages = { dashboard:<Dashboard/>, orders:<Orders/>, debts:<Debts/>, shipments:<Shipments/>, clients:<Clients/>, products:<Products/> };
        ...
        <div style={S.app}>
          <div style={S.topbar}>
            ...
            <div style={S.topRight}>
              ...
              <button style={{background:isLight?"#e8edf8":"#ffffff15",border:`1px solid ${C.border}`,borderRadius:20,padding:"5px 11px",fontSize:13,cursor:"pointer",color:C.text}} onClick={()=>setIsLight(!isLight)}>{isLight?"🌙":"☀️"}</button>
