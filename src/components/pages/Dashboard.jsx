            ...
            <table className="w-full text-sm min-w-[420px]">
              <thead>
                <tr className="text-left text-brand-blue/40 border-b border-brand-blue/5">
                  <th className="pb-3 font-bold uppercase tracking-widest text-[10px] w-24">{T.orderId}</th>
                  <th className="pb-3 font-bold uppercase tracking-widest text-[10px]">{T.client}</th>
                  <th className="pb-3 font-bold uppercase tracking-widest text-[10px] w-24 text-right">{T.total}</th>
                  <th className="pb-3 font-bold uppercase tracking-widest text-[10px] w-24 text-right">{T.status}</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-brand-blue/5">
                {orders.slice(0, 5).map(o => (
                  <tr key={o.id} className="group hover:bg-white/40 transition-colors">
                    <td className="py-4 font-bold text-brand-blue whitespace-nowrap">{o.id}</td>
                    <td className="py-4 font-medium truncate max-w-[100px]">{o.client}</td>
                    <td className="py-4 font-bold text-brand-blue whitespace-nowrap text-right">${o.total.toLocaleString()}</td>
                    <td className="py-4 text-right">
                      <span className={`
                        px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider whitespace-nowrap inline-block
